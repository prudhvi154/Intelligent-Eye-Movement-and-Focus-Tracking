...
export class EyeTracker {
  ...
  calculateHeadPose(landmarks) {
    ...
    const pitch = (pitchRatio - 0.45) * 2.2; 
  ...
  determineFocusState(gazeScreen, headPose, blinkValue, numFaces) {
    if (numFaces > 1) {
      return { focused: false, status: "Multiple Faces", code: "MULTIPLE_FACES", reason: `${numFaces} individuals detected in frame.` };
    }
    if (blinkValue > this.thresholds.blinkThreshold) {
      return { focused: false, status: "Eyes Closed", code: "EYES_CLOSED", reason: "User closed their eyes or is looking straight down." };
    }
    if (Math.abs(headPose.yaw) > this.thresholds.yawLimit) {
      return { focused: false, status: "Head Turned Away", code: "HEAD_TURNED", reason: headPose.yaw > 0 ? "Looking far right." : "Looking far left." };
    }
    if (Math.abs(headPose.pitch) > this.thresholds.pitchLimit) {
      return { focused: false, status: "Head Tilted", code: "HEAD_TILTED", reason: headPose.pitch > 0 ? "Looking down." : "Looking up." };
    }

    const offScreenX = gazeScreen.x < -0.1 || gazeScreen.x > 1.1;
    const offScreenY = gazeScreen.y < -0.1 || gazeScreen.y > 1.1;
    if (offScreenX || offScreenY) {
      return { focused: false, status: "Gaze Off-Screen", code: "GAZE_AWAY", reason: "Eyes drifted away from the active window." };
    }

    return { focused: true, status: "Focused", code: "FOCUSED", reason: "User is actively looking at the screen." };
  }
}
